...
const DEFAULT_SPOTLIGHT_RADIUS = 450;
const DEFAULT_GLOW_COLOR = '20, 20, 168';
...
const createParticleElement = (x: number, y: number, color: string = DEFAULT_GLOW_COLOR): HTMLDivElement => {
  ...
  el.style.cssText = `
    ...
    box-shadow: 0 0 15px rgba(${color}, 0.9);
    filter: blur(1px);
    pointer-events: none;
    z-index: 100;
    left: ${x}px;
  ...
  useEffect(() => {
    ...
    spotlight.style.cssText = `
      ...
      background: radial-gradient(circle,
        ...
        rgba(${glowColor}, 0.25) 20%,
        rgba(${glowColor}, 0.15) 40%,
        rgba(${glowColor}, 0.08) 60%,
        rgba(${glowColor}, 0.03) 75%,
        transparent 85%
      );
      z-index: 200;
      opacity: 0;
    ...
    <>
        ...
        {`
          ...
          .card--border-glow::after {
            ...
            background: radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y),
                ...
                rgba(${glowColor}, calc(var(--glow-intensity) * 0.8)) 25%,
                rgba(${glowColor}, calc(var(--glow-intensity) * 0.4)) 50%,
                transparent 85%);
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {(customCards || defaultCardData).slice(0, 5).map((card, index) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="card__content flex flex-col relative text-white z-[2]">
                        {card.description && (
                            <span
                            className={`card__tag inline-block w-fit text-xs font-medium rounded-full px-3 py-1 mt-auto ${
                                card.description.toLowerCase() === t('hospitality').toLowerCase()
                                ? 'bg-white text-[#030326]'
                                : card.description.toLowerCase() === t('logistics').toLowerCase()
                                ? 'bg-[#07073D] text-white'
                                : 'bg-skwd-text-highlight text-white'
                            }`}
                            >
                            {card.description}
                            </span>
              ...
              <div
                ...
                ref={el => {
                  ...
                  const handleClick = (e: MouseEvent) => {
                    ...
                    ripple.style.cssText = `
                      position: absolute;
                      width: ${maxDistance * 3}px;
                      height: ${maxDistance * 3}px;
                      border-radius: 50%;
                      background: radial-gradient(circle, rgba(${glowColor}, 0.4) 0%, rgba(${glowColor}, 0.8) 40%, transparent 10%);
                      left: ${x - maxDistance}px;
                      top: ${y - maxDistance}px;
                      pointer-events: none;
                      z-index: 1000;
